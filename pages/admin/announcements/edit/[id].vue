    ...
    <NuxtLayout name="admin">
        <div class="p-4 md:p-8 max-w-4xl mx-auto">
            <div class="mb-6">
                ...
                <h1 class="text-2xl md:text-3xl font-bold text-slate-800">📝 Edit Announcement</h1>
            </div>
            ...
            <form v-else-if="announcement" @submit.prevent="handleSubmit"
...
const { data: response, pending } = await useFetch<ApiResponse>(`/api/announcements`)
const announcement = computed(() => {
    const announcements = response.value?.data || []
    return announcements.find((a) => a._id === route.params.id)
})
...
watch(announcement, (newVal) => {
    if (newVal) {
        form.value = {
            ...
            image: newVal.image || '',
            link: newVal.link || '',
            linkText: newVal.linkText || 'ดูเพิ่มเติม',
            isActive: newVal.isActive,
            startDate: new Date(newVal.startDate).toISOString().slice(0, 16),
            endDate: newVal.endDate ? new Date(newVal.endDate).toISOString().slice(0, 16) : '',
            showOnce: newVal.showOnce,
            priority: newVal.priority || 0
        }
    }
}, { immediate: true })
...
const handleSubmit = async () => {
    ...
    try {
        await $fetch(`/api/announcements/${route.params.id}`, {
            method: 'PUT',
            body: {
                ...form.value,
        ...
        showAlert(e.data?.message || 'Failed to update announcement', 'error')
    } finally {
